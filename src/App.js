...
const App = () => {
  ...
  useEffect(() => {
    const SessionAndLocalState = async () => {
      ...
      } else if (LocalStorage.get("UserToken")) {
        if (cookiesOption.get("refresh_token")) {
  ...
  return (
    <BrowserRouter>
      ...
      <Routes>
        ...
        <Route
          path="/board"
          element={<BulletinBoard setHeader={setHeader} />}
        />
        <Route
          path="/board/write"
        ...
        <Route
          path="/board/edit"
          element={<EditPost setHeader={setHeader} />}
        />
      </Routes>
    </BrowserRouter>
  );
