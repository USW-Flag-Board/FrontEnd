...
const ActivityEdit = () => {
  ...
  const submit = async () => {
    ...
    try {
      const response = await instance.put(`/activities/${activityId}`, data);
      if (response.status === 200) {
        alert("활동이 수정되었습니다");
        navigate(`/activity/content/${activityId}`);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const activityData = response.data.payload;
        setType(activityData.type);
        setTitle(activityData.name);
        setContent(activityData.description);
        setGithubLink(activityData.githubURL);
        setBookUsage(activityData.bookUsage);
        setbookName(activityData.bookName);
        setProceed(activityData.proceed);
        editorRef.current?.getInstance().setMarkdown(activityData.description);
      } catch (error) {
        console.log(error);
      }
    }
    ...
    <div>
      ...
      <ContentArea>
        <ContentBox>
          ...
          <ContentButtonBox>
            <ContentButton
              onClick={() => navigate(`/activity/content/${activityId}`)}
            >
              취소
            </ContentButton>
            <ContentButton onClick={submit}>수정</ContentButton>
          </ContentButtonBox>
        </ContentBox>
      </ContentArea>
    </div>
